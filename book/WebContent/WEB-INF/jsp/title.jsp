...
<script>
	...
	function setCookies(){
		var username=document.getElementById("name").value;
		var password = document.getElementById("password").value;
		var radio = document.getElementById("identity").value;
		var checked = $("input[type='checkbox']").is(':checked');//获取是否需要记住账号
		if(checked){
			var date=new Date();
			...
			$.cookie("radio",radio,{expires:date,path:'/'});
		}else{
	...
	function getCookies(){
		...
		if(radio=="dean"){
			$("[name='dean']").attr("checked",true);
		}else{
			$("[name='teacher']").attr("checked",true);
		}
		
		var date = new Date();
		var year=date.getYear()+1900;
		var copyright = document.getElementById("copyright");
		var cr = "Copyright &copy; 2013-"+year+"果拉文化版权所有";
		copyright.innerHTML=cr;
	}
