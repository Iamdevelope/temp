...
$(document).ready(function(){
	...
	$('#downloadModel').click(function(){
		window.location.href = "${pageContext.request.contextPath}/modelDownload?templateId=teachingplan";
	});
	$('#downloadData').click(function(){
		var btype = $('input#btype').val();
		window.location.href = "${pageContext.request.contextPath}/teachingPlan?templateDownId=teachingplan&btype="+btype;
	});
	...
	$('#importWordBtn').on('click',function(){
		window.location.href = "${pageContext.request.contextPath}/teachingPlan_upload";
	...
	$('#addPage').on('click',function(){
		...
		//window.location.href = "${pageContext.request.contextPath}/teachingPlan_addPage?btype="+type;
	});
...
function check_book_data(){
	console.log("点击了确定按钮");
	var book_name=document.getElementById("teachingPlan_save_do_book_name").value;
	var book_theme=document.getElementById("teachingPlan_save_do_book_theme").value;
...
<div>
								...
								<c:forEach var="b" items="${pageBean.list}">
									<tr onmouseover="this.style.backgroundColor = 'white'"
										...
										<c:if test="${existAdmin.isadmin==0 }">
											...
											href="${ pageContext.request.contextPath }/teachingPlan_delete?idteaching_plan=${b.idteaching_plan }&btype=${b.booktype.idbooktype}">
	...
	<div class="easyui-dialog" title="导入Excel数据" id="importModel" draggable=true modal=true closed=true style="display:black;width:350px;height:200px" >
		<form id="importForm" action="${pageContext.request.contextPath }/teachingPlan_importData" enctype="multipart/form-data" method="post">
    		<table>
    			<tr>
    				<td>浏览:</td>
    				<td>
	...
	<div class="easyui-dialog" title="导入绘本教案" id="importWordPanel" draggable=true modal=true closed=true style="display:black;width:720px;height:580px" >
		<form id="importForm" action="${pageContext.request.contextPath }/teachingPlan_importWordData" enctype="multipart/form-data" method="post">
    		<table>
    			<tr>
    				<td>浏览:</td>
    				<td>
    					<input id="wordInput" class="m_upload" name="wordInput" type="file" />
    				</td>
    			</tr>
    			<tr>
    				<td colspan="2">
	...
	<div class="easyui-dialog" title="新增绘本数据" id="addModel" draggable=true modal=true closed=true style="display:black;width:630px;height:315px" >
		...
		action="${pageContext.request.contextPath}/teachingPlan_save?btype=<s:property value="type"/>"
		...
		<table cellSpacing="1" cellPadding="5" width="100%" align="center"
			...
			<tr>
				...
				<td class="ta_01" bgColor="#ffffff"><input type="text"
					name="name" value="" id="teachingPlan_save_do_book_name"
					class="bg" /></td>
			</tr>
			<tr>
				<td width="25%" align="center" bgColor="#f5fafe" class="ta_01">
				...
				<td class="ta_01" bgColor="#ffffff">
					<input type="text"
					name="book_type" value="<s:property value="currentType"/>" id="teachingPlan_save_do_book_name"
			...
			<tr>
				...
				<td class="ta_01" bgColor="#ffffff"><input type="text"
					name="book_theme" value="" id="teachingPlan_save_do_book_theme"
					class="bg" /></td>
			</tr>
			<tr>
				<td width="25%" align="center" bgColor="#f5fafe" class="ta_01">
			...
			<tr>
				<td class="ta_01" style="WIDTH: 100%" align="center"
					...
					<button type="submit" id="teachingPlan_save_do_submit"
	...
	<div id="mm1" style="width:8%;">
		<div id="downloadModel">
			<a  title="下载教案Excel表模板" >下载Excel模板</a>
		</div>
		<div id="downloadData">
			<a title="下载教案信息">导出数据</a>
		</div>
	</div>
	<div id="mm2" style="width:12%;">
		<div id="importBtn">
			<a title="导入教案信息">导入教案信息</a>
		</div>
		<div id="importWordBtn">
			<a title="导入绘本教案信息">上传教案</a>
		</div>
	</div>
</body>
</html>