$(document).ready(function() {
    ...
    //check book_context
    if (!book_context || book_context.length == 0) {
        alert('book_context is null');
        return;
    }
    ...
    left_bar.click(function() {
        if (current - 2 >= 0) {
            ...
            flip.find('.content').html(book_context[current + 1]);
            ...
            if (isIE) {
                ...
                window.setTimeout(function() {
                    if (book_context[current]) {
                        left.find('.content').html(book_context[current]).show();
                    } else {
                ...
                window.setTimeout(function() {
                    if (book_context[current]) {
                        left.find('.content').hide().html(book_context[current]).css({ opacity: 0.5, display: 'block' }).animate({ opacity: 1 }, (flip_time - (flip_time / 4)));
                    } else {
            ...
            flip.stop().css({
                ...
                left.find('.pager').html((current) + " / " + Math.ceil(total));
    ...
    right_bar.click(function() {
        if (current + 1 <= total - 1) {
            ...
            flip.find('.content').html(book_context[current]);
            flip.find('.pager').html((current)  + " / " + Math.ceil(total));
            flip.find('.overlayer').css({ 'width': '100%', 'margin-left': 0 });

            if (isIE) {
                right.find('.content').hide();
                window.setTimeout(function() {
                    if (book_context[current + 1]) {
                        right.find('.content').html(book_context[current + 1]).show();
                    } else {
                ...
                window.setTimeout(function() {
                    if (book_context[current + 1]) {
                        right.find('.content').hide().html(book_context[current + 1]).css({ opacity: 0.5, display: 'block' }).animate({ opacity: 1 }, (flip_time - (flip_time / 4)));
                    } else {
    ...
    //glide
    var step = parseInt($('.glide').width() / (Math.ceil(total) - 1));
    var slider = $('.glide').slider({
        min: step,
        max: $('.glide').width(),
        step: step * 2,
        slide: function(event, ui) {
            $('.glide a').attr('title', (ui.value / step + 1) + '/' + Math.ceil(total));
